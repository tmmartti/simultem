...
def patch_trial(patch_url_base, minStay, headers, result_list, successful_patches, failed_patches):
    patch_url = patch_url_base[:-1]
    if patch_url != 'https://api.apaleo.com/rateplan/v1/rate-plans?ratePlanIds':
        patch_url = patch_url_base[:-1]
        new_value = description(minStay)
...
def patch_rate_plans(token, rate_plans, successful_patches, failed_patches):
    ...
    for rate_plan in rate_plans:
        ...
        if minStay[0]['value']['minLengthOfStay'] != 28:
            ...
            minStay_28 = minStay
            patch_url_base_28 += rate_plan_id + ',' 
    try:
        patch_trial(patch_url_base_0, minStay_0, headers, under28, successful_patches, failed_patches)
    except:
        print ('No under 28s')
    try:
        patch_trial(patch_url_base_28, minStay_28, headers, over28, successful_patches, failed_patches)
    except:
        print ('No 28s')
...
failed = ['CENA-FLEX5APARTNER', 'CENA-FLEX5BPARTNER']

custom_rule = lambda elem: elem['property']['id'] not in ['TURKU1'] and 'CC' not in elem['code'] and elem['id'] in failed and partnerCheck(elem)
...
script_runner(1, 200, successful_patches, failed_patches)
